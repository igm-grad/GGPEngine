...
bool RenderEngine::InitDefaultMaterial() 
{
	...
	defaultMaterial = new Material(device, deviceContext, L"VertexShader.cso", L"PixelShader.cso");
...
void RenderEngine::Update(float totalTime, std::vector<GameObject*> list) 
{
	...
	for (GameObject* g : list) {
		...
		defaultMaterial->sVertexShader->SetMatrix4x4("world", world);
		defaultMaterial->sVertexShader->SetMatrix4x4("view", defaultCamera->view);
		defaultMaterial->sVertexShader->SetMatrix4x4("projection", defaultCamera->projection);
		defaultMaterial->sVertexShader->SetShader();
		defaultMaterial->sPixelShader->SetShader();
